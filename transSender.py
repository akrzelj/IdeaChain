...
def SendSingleTransactionsToAllNodes(data):
    for i in bChainServerList:
    # Create a TCP/IP socket
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

        # Connect the socket to the port where the server is listening
        server_address = (i, 9999)
...
def SendTransactionstoAllNodes(transactionQueue):
    for i in transactionQueue:
        print(i)
        SendSingleTransactionsToAllNodes(i)
    end = transaction("end", "end")
    SendSingleTransactionsToAllNodes(end)

SendTransactionstoAllNodes(transactionQueue)
